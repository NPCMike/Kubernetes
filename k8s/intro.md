...
今天不用背完整本 Kubernetes 百科全書。你只要跟著任務走，學會五件事：

- 現場有哪些機器在工作？
- 哪個服務看起來不正常？
- Kubernetes 為什麼能把壞掉的服務補回來？
- 資源要怎麼分區，服務要怎麼有穩定入口？
- 設定和敏感資訊要怎麼交給 Pod，而不是寫死在 image 裡？
...
Stage 4  自動補位：用 Deployment 維持服務副本數
  |
  v
Stage 5  現場分區：用 Namespace 把資源放到正確區域
  |
  v
Stage 6  穩定入口：用 Service 找到活著的 Pod
  |
  v
Stage 7  設定外掛：用 ConfigMap 管理非敏感設定
  |
  v
Stage 8  敏感資料：用 Secret 管理 token 和密碼
```
